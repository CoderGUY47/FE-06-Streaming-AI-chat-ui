...
export async function POST(req: Request) {
  try {
    ...
    const dynamicSystem = `Today's Date: ${new Date().toISOString().split("T")[0]} (Year 2026). MANDATE: State the direct, accurate answer immediately on line 1 without preambles. Keep responses concise, simple, and brief.\n\n${SYSTEM_PROMPT}`;
    ...
    const result = streamText({
      ...
      tools: {
        getRecentNews: tool({
          description: "Search the web for real-time news, current events, recent movie releases, technology updates, or facts after 2024.",
          inputSchema: z.object({
            query: z.string().describe("The search query to look up live news or facts"),
          }),
